...
export const retrieveUser = createEffect(
  (auth = inject(Auth)) =>
    new Observable<User | null>((subscriber) => onAuthStateChanged(auth, subscriber)).pipe(
      map((user) => {
        if (user) {
          const currentUser = mapFirebaseUser(user);
          return AuthActions.retrieveUserSuccess({ user: currentUser });
        }
    ...
    actions$.pipe(
      ...
      switchMap(({ email, password }) =>
        from(signInWithEmailAndPassword(auth, email, password)).pipe(
          map((userCredential) =>
            AuthActions.signInSuccess({ user: mapFirebaseUser(userCredential.user) }),
          ),
